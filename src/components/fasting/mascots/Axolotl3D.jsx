...
/**
 * Axolotl3D — cara estilo Apple Animoji
...
export default function Axolotl3D({ phaseColor, darkPhaseColor, glowColor, state, reaction, phaseIndex }) {
  ...
  // Branquia helper — sprota hacia afuera desde los lados de la cara
  ...
  function Gill({ side }) {
    ...
    return (
      <group position={[side * 0.86, 0.26, 0]}>
        {angles.map((angle, i) => (
          <group key={i} position={[side * xOffsets[i], 0, 0]} rotation={[0, 0, angle * side]}>
            <mesh>
