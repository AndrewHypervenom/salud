...
export default function Axolotl3D({ phaseColor, darkPhaseColor, glowColor, state, reaction, phaseIndex }) {
  ...
  useFrame((_, delta) => {
    ...
    if (reaction === 'dancing') {
      rootRef.current.position.y = Math.abs(Math.sin(t * 6)) * 0.18
      rootRef.current.rotation.y = Math.sin(t * 4) * 0.4
      rootRef.current.rotation.z = Math.sin(t * 6) * 0.08
      ...
      if (waveRef.current) waveRef.current.rotation.z = Math.sin(t * 8) * 0.6 - 0.2
    } else {
      // idle / sleeping
      const speed = sleeping ? 0.8 : 1.4
      const amp = sleeping ? 0.02 : 0.055
        ...
        {angles.map((angle, i) => (
          <group key={i} position={[xOff[i] * (flip ? -1 : 1), 0, 0]} rotation={[0, 0, angle * (flip ? -1 : 1)]}>
            ...
            <mesh position={[0, heights[i] / 2 + 0.06, 0]}>
              ...
              <meshStandardMaterial color={branchiaColor} roughness={0.2} metalness={0.05} emissive={branchiaColor} emissiveIntensity={0.15} />
            </mesh>
          </group>
        ))}
      </group>
  ...
  return (
    <group ref={rootRef} rotation={[0, -0.3, 0]}>
      ...
      <mesh rotation={[0, 0, Math.PI / 2]}>
        ...
        <meshStandardMaterial ref={bodyMatRef} color={phaseColor} roughness={0.15} metalness={0.05} />
      </mesh>

      {/* Cabeza */}
      <mesh position={[0.68, 0.1, 0]}>
        <sphereGeometry args={[0.44, 20, 20]} />
        <meshStandardMaterial ref={headMatRef} color={phaseColor} roughness={0.15} metalness={0.05} />
      </mesh>
      ...
      <mesh ref={waveRef} position={[0.42, -0.44, -0.32]} rotation={[0.3, 0, 0.45]}>
        ...
        <meshStandardMaterial ref={legMatRef} color={darkPhaseColor} roughness={0.2} metalness={0.03} />
      </mesh>
      {/* Pata delantera derecha */}
      <mesh position={[0.42, -0.44, 0.32]} rotation={[-0.3, 0, 0.45]}>
        <capsuleGeometry args={[0.075, 0.28, 5, 8]} />
        <meshStandardMaterial color={darkPhaseColor} roughness={0.2} metalness={0.03} />
      </mesh>
      ...
      <mesh position={[-1.0, 0.05, 0]} rotation={[0, 0, Math.PI / 2]} scale={[1, 0.55, 0.6]}>
        ...
        <meshStandardMaterial ref={tailMatRef} color={darkPhaseColor} roughness={0.15} metalness={0.05} />
      </mesh>
