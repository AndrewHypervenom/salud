    ...
    <>
      <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-xl font-bold text-primary-600">💚 {t('app_title')}</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2 py-1.5 rounded-xl text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors text-lg"
      ...
      {showSheet && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end md:justify-center md:items-center" onClick={() => setShowSheet(false)}>
          ...
          <div
            className="relative bg-white rounded-t-3xl md:rounded-2xl w-full md:max-w-sm p-6 max-h-[80vh] overflow-y-auto"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">{t('profile.select')}</h2>
